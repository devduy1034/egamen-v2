...
namespace LARAVEL\Helpers\Clockwork\Datasource;

use Clockwork\DataSource\Concerns\EloquentDetectDuplicateQueries;
use Clockwork\DataSource\DataSource;
use Clockwork\Helpers\Serializer;
use Clockwork\Helpers\StackTrace;
use Clockwork\Request\Request;
use Illuminate\Contracts\Events\Dispatcher as EventDispatcher;
use LARAVEL\DatabaseCore\DatabaseManager;
use LARAVEL\DatabaseCore\Eloquent\MissingAttributeException;
...
use LARAVEL\Helpers\Clockwork\Eloquent\ResolveModelScope;
...
{
	use EloquentDetectDuplicateQueries;
	...
	public function __construct(DatabaseManager $databaseManager, EventDispatcher $eventDispatcher, $collectQueries = true, $slowThreshold = null, $slowOnly = false, $detectDuplicateQueries = false, $collectModelsActions = true, $collectModelsRetrieved = false)
	{
	...
	{
		// echo self::class .  ' - resolve()' . \PHP_EOL;
	...
	{
		// echo self::class .  ' - listenToEvents()' . \PHP_EOL;
		...
		if ($scope = $this->getModelResolvingScope()) {
			$this->eventDispatcher->listen('eloquent.booted: *', function ($model, $data = null) use ($scope) {
				...
				// echo self::class .  ' - eloquent.booted' . \PHP_EOL;
	...
	{
		// if (interface_exists(ScopeInterface)) {
		// 	// Laravel 5.0 to 5.1

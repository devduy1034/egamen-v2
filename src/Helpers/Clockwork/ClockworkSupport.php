...
class ClockworkSupport
{
	...
	public function getData($id = null, $direction = null, $count = null, $filter = [], $extended = false)
	{
		...
		// 	return \response()->json([ 'message' => $authenticated, 'requires' => $authenticator->requires() ], 403);
		// }
	...
	public function updateData($id, $input = [])
	{
		...
		if (! $request->updateToken || ! hash_equals($request->updateToken, $token)) {
			return \response()->json(['message' => 'Invalid update token.'], 403);
		}
	...
	public function makeStorage()
	{
		...
		if ($storage == 'sql') {
			$database = $this->getConfig('storage_sql_database', \cache_path('clockwork.sqlite'));
			...
			return new FileStorage(
				$this->getConfig('storage_files_path', \cache_path('clockwork')),
				0700,
				$expiration,
	...
	public function processRequest($request, $response)
	{
		...
		header('X-Clockwork-Id: ' . $clockworkRequest->id, true);
		header('X-Clockwork-Version: ' . Clockwork::VERSION, true);

		if ($request->getBasePath()) {
			header('X-Clockwork-Path: ' . $request->getBasePath() . '/__clockwork/', true);
		}

		foreach ($this->getConfig('headers', []) as $headerName => $headerValue) {
			header("X-Clockwork-Header-{$headerName}: {$headerValue}", true);
		}
		...
		foreach ($clockwork->request()->subrequests as $subrequest) {
			...
			header('X-Clockwork-Subrequest: ' . "{$subrequest['id']};{$url};{$path}", false);
		}
	...
	public function recordRequest()
	{
		...
		$clockwork->getStorage()->cleanup(true);
	...
	public function isEnabled()
	{
		return $this->getConfig('enable')
			...
			&& ($this->incomingRequest()->hasLocalHost()
	...
	protected function appendServerTimingHeader($response, $request)
	{
		if (($eventsCount = $this->getConfig('server_timing', 10)) !== false) {
			header('Server-Timing: ' . ServerTiming::fromRequest($request, $eventsCount)->value());
		}
	}
	...
	{
		LARAVELRouter::get('/__clockwork/{id}/extended', '\LARAVEL\Helpers\Clockwork\ClockworkController@getExtendedData')
			->where(['id' => '([0-9-]+|latest)']);
		LARAVELRouter::get('/__clockwork/{id}/{direction?}/{count?}', '\LARAVEL\Helpers\Clockwork\ClockworkController@getData')
			->where(['id' => '([0-9-]+|latest)', 'direction' => '(next|previous)', 'count' => '\d+']);
		LARAVELRouter::put('/__clockwork/{id}', '\LARAVEL\Helpers\Clockwork\ClockworkController@updateData');
		LARAVELRouter::post('/__clockwork/auth', '\LARAVEL\Helpers\Clockwork\ClockworkController@authenticate');
	}
}

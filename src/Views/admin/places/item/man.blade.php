            ...
            <table class="datatables-category-list table border-top text-sm">
                ...
                @forelse($items as $k => $v)
                    ...
                    <td class="align-middle w-[70px] !pl-0">
                        @component('component.inputNumb',['numb'=>$v['numb'],'idtbl'=>$v['id'],'table'=>'ward'])@endcomponent
                    </td>
                    <td class="align-middle">
                        ...
                        <div class="tool-action mt-2 w-clear">
                            @component('component.buttonAction',['config'=>$configMan->categories->item,'slug'=>$v['slugvi'],'params'=>['id'=>$v['id'],'id_city'=>$v['id_city'],'id_district'=>$v['id_district']]])
                                @if (!empty($configMan->categories->item->copy_categories))
                                    <div class="dropdown">
                                        <a id="dropdownCopy" data-url="{{ url('copy') }}" data-id="{{ $v['id'] }}" data-table="ward" data-com="{{ $com }}" data-type="{{ $type }}" class="nav-link text-success mr-3"><i class="ti ti-copy"></i>Copy</a>
                                    </div>
                                @endif
                            @endcomponent
                        </div>
                        ...
                        @foreach ($configMan->categories->item->status_categories as $key => $value)
                            ...
                            <td class="align-middle text-center">
                                <label class="switch switch-success">
                                    @component('component.switchButton',['keyC'=>$key,'idC'=>$v['id'],'tableC'=>'ward','status_arrayC'=>$status_array]) @endcomponent
                                </label>
                            </td>
                        @endforeach
                    @endif
